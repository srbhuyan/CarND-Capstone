...
def plot_wp_vc_sl_tl_2D(waypoints,pose,config,lights):
    '''
    Plot way points, the ego car, and the stop line
    '''
    # Plot the way points
    ...
    plt.scatter(x_wp, y_wp, marker='^', s=20, c=c_wp, alpha=0.5, 
                edgecolors='none', cmap=plt.cm.cool)
    # Plot the car
    x_vc = [pose.pose.position.x]
    y_vc = [pose.pose.position.y]
    plt.scatter(x_vc, y_vc, marker='s', s=50, c='b', alpha=0.5, edgecolors='none')
    # Plot the stop line
    stop_line_positions = config['stop_line_positions']
    x_sl = []
    y_sl = []
    for p in stop_line_positions:
        x_sl.append(p[0])
        y_sl.append(p[1])
    plt.scatter(x_sl, y_sl, marker='|', s=500, c='gray', alpha=0.9, edgecolors='none')
    # Plot traffic lights
    x_tl = []
    y_tl = []
    c_tl = []
    for light in lights:
        state = light.state
        x_tl.append(light.pose.pose.position.x)
        y_tl.append(light.pose.pose.position.y)
        if state == 2:
            c_tl.append('g')
        elif state == 1:
            c_tl.append('y')
            ...
            c_tl.append('gray')
    plt.scatter(x_tl, y_tl, marker='o', s=100, c=c_tl, alpha=0.2, edgecolors='none')
    plt.savefig('wp_vc_sl_tl_2D.png')
    plt.close('all')